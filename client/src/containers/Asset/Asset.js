...
class Asset extends Component {
  ...
  onTermSubmit = term => {
    ...
    fetch(`${process.env.REACT_APP_URL}/api/asset/all/${term}`, {
      method: 'POST',
      body: JSON.stringify({
        userId: localStorage.getItem(`userId`)
      }),
  ...
  render() {
    ...
    const currentAssets = this.props.allAssets.slice(
      indexOfFirstAssets,
      indexOfLastAssets
    );
    ...
    for (
      let i = 1;
      i <= Math.ceil(this.props.allAssets.length / assetsPerPage);
      i++
    ) {
      pageNumbers.push(i);
    }
    const pageNumbersEl = pageNumbers.map(number => {
      return (
        <li
          ...
          className={this.state.currentPage === number ? classes.Active : ``}
        >
          {number}
        </li>
      );
    ...
    return (
      <Fragment>
        <Search token={this.props.token} onFormSubmit={this.onTermSubmit} />
